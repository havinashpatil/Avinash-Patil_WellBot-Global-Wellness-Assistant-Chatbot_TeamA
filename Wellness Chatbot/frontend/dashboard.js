...
document.addEventListener('DOMContentLoaded', () => {
 ...
 if (moodCtx) {
         ...
         y: {
           ticks: {
             callback: function(value) {
               const map = {0:'Neutral',1:'Happy',2:'Sad',3:'Stressed',4:'Angry',5:'Tired',6:'Concerned'};
   ...
   async function updateMoodChart() {
     try {
       const res = await fetch(`/api/user_stats?email=${localStorage.getItem('email')}`);
       const data = await res.json();
       if (data.success && data.mood_history) {
         const moods = data.mood_history.reverse(); // oldest to newest
         const labels = moods.map((_, i) => `Day ${i+1}`);
         const moodMap = {'Neutral':0,'Happy':1,'Sad':2,'Stressed':3,'Angry':4,'Tired':5,'Concerned':6};
         const values = moods.map(m => moodMap[m] ?? 0);
...
// Render Mood History Chart (Overview)
const moodCtx2 = document.getElementById('moodChart');
if (moodCtx2) {
  const moodChart2 = new Chart(moodCtx2, {
    type: 'line',
    data: {
      labels: [],
      datasets: [{
  ...
  // Update chart with real data (reuse same fetch)
  async function updateMoodChart2() {
    try {
      const res = await fetch(`/api/user_stats?email=${localStorage.getItem('email')}`);
      const data = await res.json();
      if (data.success && data.mood_history) {
        const moods = data.mood_history.reverse();
        const labels = moods.map((_, i) => `Day ${i+1}`);
        const moodMap = {'Neutral':0,'Happy':1,'Sad':2,'Stressed':3,'Angry':4,'Tired':5,'Concerned':6};
        const values = moods.map(m => moodMap[m] ?? 0);
        moodChart2.data.labels = labels;
        moodChart2.data.datasets[0].data = values;
        moodChart2.update();
      }
    } catch (e) { console.error(e); }
  }
...
window.submitIssueReport = async function () {
    ...
    const msgBox = document.getElementById('issueSubmitMsg');

    if (!type) {
        msgBox.innerHTML = '<p style="color:#EF4444;"><i class="fa-solid fa-circle-exclamation"></i> Please select an issue type.</p>';
        return;
    }
    if (!description) {
        msgBox.innerHTML = '<p style="color:#EF4444;"><i class="fa-solid fa-circle-exclamation"></i> Please describe the issue.</p>';
        return;
    }

    msgBox.innerHTML = '<p style="color:var(--text-muted);"><i class="fa-solid fa-spinner fa-spin"></i> Submitting...</p>';
    ...
    try {
        const res = await fetch('/api/report-issue', {
            ...
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                email: localStorage.getItem('email') || 'Anonymous',
        ...
        if (data.success) {
            msgBox.innerHTML = '<p style="color:#10B981;"><i class="fa-solid fa-check-circle"></i> Issue reported successfully! Our team will look into it.</p>';
            document.getElementById('issueType').value = '';
            document.getElementById('issueDescription').value = '';
        } else {
            msgBox.innerHTML = `<p style="color:#EF4444;"><i class="fa-solid fa-circle-xmark"></i> Failed: ${data.error || 'Unknown error'}</p>`;
        }
    } catch (err) {
        msgBox.innerHTML = '<p style="color:#EF4444;"><i class="fa-solid fa-circle-xmark"></i> Connection error. Please try again.</p>';
    }
};
